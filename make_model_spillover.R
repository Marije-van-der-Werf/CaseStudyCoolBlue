...
source("dummies.R")
source("create_modeldata_table_spillover.R")
...
x <- x %>% 
    mutate(mobileDesktopS_FBAndInsta = mobileDesktopS * FacebookAndInstagram,
           ...
           salesCampaign_buyDirect = sales_campaign * buy_direct
    )

x <- data.matrix(x)
D <- data.matrix(D)
adX <- x[, c(1:24, 27:29)] # Hier alle x'en die in de adstock gaan
noAdX <- x[, c(25, 26, 30:62)] # Hier alle x'en die niet in de adstock gaan # Kosten van tv en laptop staan hier nu in
